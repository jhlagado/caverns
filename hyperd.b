POKE 162,30: POKE 163,128: POKE 140,1: POKE 220,23: RESTORE 20000: FOR X=0 TO 41: READ Y: POKE 62976+X,Y: NEXT X: ON  ERRORGOTO 19999
...
300  IF Z=1 THEN  PRINT "You have been stung by a slug. Thousands of others crawl over   and through your c INVERSE orpse. You are a very tasty meal!": GOTO 11000
...
340  IF  RND <.3 AND A<>27 THEN  PRINT "You've just walked into the fangs of something green and sli
28025 !": GOTO 11000
350  GOTO 990
360
33570 ou are";
...
380  IF A=2 THEN  PRINT " stan3ding by the docking bay of a huge space"\"liner. There is a door to the east."
...
420  IF A=6 THEN  PRINT " are in a small plastic bubble, east of the main"\"observation deck. You can get a fantastic view of the warp"\"engines f RETURN rom here."
...
440  IF A=8 OR A=17 OR A=18 OR A=23 OR A=25 OR A=31 OR A=36 OR A=37 OR A=46 OR A=47 ELSE  THEN  PRINT " in a corridor."
...
480  IF A=12 THEN  PRINT " i INPUT n a north-south corridor."
...
500  IF A=15 THEN  PRINT " in the central library. There is a machine here for"\"playing m HIRES agnetic disks."
...
550  IF A=20 THEN  PRINT " standing in a room. To the north you can just se PLAY e a"\"faint beam of light across your path. To the east is a sign:"\"COMPUTER ROOM"
...
620  IF A=27 THEN  PRINT " in the central SET  power station. There is a big red button on the wall."
...
820  IF A=49 THEN  PRINT " in the Royal ChambGer of the Xpetian colony. You can see a ladder."
...
830  IF Z7=1 THEN 840 ELSE  IF A=47 OR A=38 OR A=37 OR A=31 THEN  PRINT "sThe room is infested with hundreds of alien green slugs which   become hungry and excited at your presence.":Z=1
...
860  IF J=A+1 THEN  PRINT "You see a patrol rob#ot pass in the distance."
...
1040  IF  SEARCH (A0$,N1$C)>0 THEN  NEXT *M 1051
...
1170 S=0: FOR X=7 TO 17: IF P(X) RENUM =-1 THEN  LET S=S+X-6
...
1280  RESTORE 2570: FOR Q=0 TO 3: READ M0$7: IF  SEARCH (A0$,M0$)>0 THEN  NEXT *Q 1300
...
1340  IF A=45 AND P(3)<>-2 THEN  PRINT "You are floating helplessly in zero gravity.": IF P(3)=-1 THEN  PRINT "Your magnetic shoes flew out of your han POINT ds and stuck to a wall!!":P(3)=0
...
1400
33857 >26 THEN 1420
1410  IF C7=0 THEN  PRINT "A powerful force- STRS field holds you back!": GOTO 270 ELSE  LET A=27: GOTO 270
1420
33857 >32 THEN 1440
...
1450  IF B=127 AND I7t=0 THEN  PRINT "BANG! You walked into an iron door!": GOTO 270: ELSE  IF B=127 THEN  LET A=22:R=0: GOTO 270
...
1600  IF M>3 THEN  PRINT "Don't be stupid!" ELSE  LET P(M)=- NORMAL 2
1610  GOTO 270
...
1700  PRINT "Species: Xpetian Angofort LLIST a"
...
1770  PRINT \\"Starship successfully completed diplomWatic mission to Algaraath system in the Q-alpha-3 sector and is now proceeding to Lethria for 3. p.revs of R & R"\\
1780  PRINT "Ship's progress: satisfactory despite a few difficulties with   Hyperdrive#2 (b.003.W) which has suffered intermittent failures.The eng TAB ineersput this down to abrasion in "
...
1800  PRINT \ SPC (18);"---- MESSAGE COMPLETE ----": SPEED  RETURN 0: INPUT A0$: CLS : GOTO 270
...
1940  IF A=31 OR A=47 OR A=38 OR A=37 THEN  PRINT "The aliens are repelled by the gas and1 move away for a moment,  but then return."
...
2000  PRINT "You unlocked the cabinet. Inside is a small magnetic di?sk markeddisk1.":P(8)=-1: GOTO 270
...
2070  INPUT "destination code";D0$:R=0: IF D0$="1" THEN  INVERSE  LET A=2
...
2160  PRINT "The safe opens. Inside is a small test tube  RETURN of Sodium Astatide.":P(7)=-1: GOTO 270
...
2260  IF A= CHR 42 THEN  LET R=0:A=43: GOTO 270
...
2320  IF A=27 THEN  PRINT ,"(check)"\"lights ";: PLAY 1,5: PRINT  SQR ,"(check)": PLAY 2;6;5;9;4;6:P7=0:R=0: GOTO 270
2330
33857 >26 THEN 2360
2340  IF  SEARCH (A0$," in ")>0 THEN  PRINT "The graphite rods move in and the whole pile cools down.": LET R7=0: GOTO 270
2350  IF  SEARCH (A0$," out ")>0 THEN  PRINT "The graphite rods withdraw slowly from the pile. The whole unit begins to melt down....";b: FOR X=1 TO 10: PLAY X,10: NEXT X: PRINT "You're dead!": GOTO 11000
2360
33857 >24 THEN 2430
...
2400 C7=1
2410  PRINT "COMPUTER SHUT DOWN": IF R7=1 THEN  PRINT "In the distance you hear the familiar rumble of an uncoBntrolled reactor overload. KABOOOM!! You're cremated!!!": GOTO 11000
2420  GOTO 270
...
2500  DATA  18,0,128,21,17,127,20,0,23,0,19,21,20,24,0,0,0, CLEAR 21,19,21,0,24,0,26,0,0,25,128,0,28,26,0,0,0,0,0,26,29,0,0,28,30,31,32,29,0,0,30,0,0,30,0,0,128,34,34,34,35,33,33,34,33
2510  DATA 33,36,33,35,38,35,0,37,0,0,36,0,0,36,0,0,40,38,38,38,4,0,0,0,0,0,42,0,0,0,0,41,0,44,0,0,43,45,0,0,48,0,46,44,0,0,47,45,0,0,0,46,0,45, SGN 49,0,0,0,0,48
...
2540  DATA " smelly test"," tube "," small magnetic disk marked"," disk1 "," small magnetic  POS disk marked"," disk2 "," monkey"," wrench "," sonic screw","driver "
2550  DATA  "",""
...
45834 > IF S<126 AND S>99 THEN  PRINT "Excellent...but you've left something behind!"
2630  IF S=126 THEN  PRINT "Perfectionist and genius!!"
2640   RETURN
3000  DATA "get",1510," put",1580," dro",1580," wea",1600," pla",1640," rea",1820,"twi",1860," tur",1860,"thr",1920,"swi",1890,"unl",1990,"cra",2020,"loo RESET",1070,"list",1090,"qui",11000
...
8226 >0 THEN  PRINT "Which dis ERROR k?": GOTO 270
...
10002  CLS : HIRES : PLOT  1,20 TO 100,50 TO 400,50 TO 500,20: PLOT  120,30 TO 120,40 TO 380,40 TO 380,30 TO 120, EXP 30: PLOT  200,50 TO 200,54 TO 220,54 TO 220,70 TO 200,70 TO 200,74 TO 220,74 TO 300,50
...
10006  CURS 470: PRINT "A  CHR W A A A Y Y Y . . . . .": CURS P: PRINT "O": PLAY 20,4: CURS P: PRINT "o": PLAY 21,3: CURS P: PRINT ".": PLAY 23,2: CURS P: PRINT " ": PLAY 24;0,6
...
10011  GOTO 11000
10212  PRINT "Help? T>hink for yourself, Dummy!": GOTO 270
11000  PRINT \\\"Another Adventure? ";
11001 A0$= KEY $: IF A0$="Y" OR A0$="y" THEN  RUN  ELSE  IF A0$<>"N" AND A0$<>"n" THEN 11001
19999  USR (62976)
20000  DATA  17,0,9,1,1,0,120,18,19,121,18,19,3,197,33,30,246,1,12,0,237,176,203,122,193,40,235,195,33,128,12,32,67,79,80,89,82,73,71,72 VAR ,84,13
